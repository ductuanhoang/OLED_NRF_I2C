...
* Typedef definitions
***********************************************************************************************************************/
#define LVGL_TIMER_MEAS_INTERVAL    APP_TIMER_TICKS(2000) // 100ms
...
static void lvgl_timer_init(void)
{
    ...
    // Start lvgl timer
    err_code = app_timer_start(m_lvgl_count_test_id, 200, NULL);
    APP_ERROR_CHECK(err_code);
...
void user_lvgl_update_number(void)
{
    ...
    lv_label_set_text(number_count, buffer);
    lv_obj_align(number_count, NULL, LV_ALIGN_IN_TOP_LEFT, 0, 0);
}
...
//
//    /*Create a Label on the currently active screen*/
//    number_count =  lv_label_create(scr, NULL);
//
//    /*Modify the Label's text*/
//
//    lv_label_set_text(number_count, "1");
//    /* Align the Label to the center
//     * NULL means align on parent (which is the screen now)
//     * 0, 0 at the end means an x, y offset after alignment*/
//    lv_obj_align(number_count, NULL, LV_ALIGN_IN_TOP_LEFT, 0, 0);
